...
DIR="/home/yaniss/infra-home/ha_config/status"
systemctl is-active ambilight-sync.service > "$DIR/ambisync.txt"
systemctl is-active openrgb-server.service > "$DIR/openrgb.txt"
ping -c1 -W2 192.168.1.26 > /dev/null 2>&1 && echo "ON" > "$DIR/tv.txt" || echo "OFF" > "$DIR/tv.txt"
...
# Pi-hole stats
PIHOLE_PASS=$(grep PIHOLE_PASSWORD /home/yaniss/infra-home/.env | cut -d= -f2)
